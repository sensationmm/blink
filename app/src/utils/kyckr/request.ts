...
const requestCompanyProfile = async (companyNumber: string, countryISOCode: string = "GB", orderReference: string, ignoreDB: boolean = false) => {
    const response = await fetch(`${domain}/kyckrCompanyProfile/${companyNumber}/${countryISOCode}/${orderReference}/${ignoreDB ? '?ignoreDB=true' : ''}`, { mode: 'cors' });
    try {
        const body = await response.json();
        return body;
    }
